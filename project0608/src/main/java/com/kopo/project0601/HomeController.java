...
//			String id = (Stirng) session.getAttribute("login_id");
	...
	public String update2Table(HttpServletRequest request, Locale locale, Model model) {
		HttpSession session = request.getSession();
		try {
			boolean isLogin = (Boolean) session.getAttribute("is_login");
			...
			if (isLogin) {			
				...
				UserDB db = new UserDB();
				People p1 = db.detailsData(idx);
				
				model.addAttribute("Origin_name", p1.id);
				model.addAttribute("Origin_address", p1.address);
	...
	@RequestMapping(value = "/update2_action", method = RequestMethod.GET)	
	public String update2Action(HttpServletRequest request, Locale locale, Model model) {
		HttpSession session = request.getSession();
		try {
			boolean isLogin = (Boolean) session.getAttribute("is_login");
